# -*- coding: utf-8 -*-
# === MQ ATMOS LAB: BELLATOR V16.4 (VISIBILITY FIX) ===
# SOLUCIÓN: Aumentado el margen inferior y subido el texto de previsión
# para evitar que el recorte de imagen se coma los datos +3H/+6H.
...
print("📡 INICIANDO SISTEMA V16.4 (VISIBILITY FIX)...")
...
def generate_ui_card(sector, data_now, data_3h, data_6h):
    ...
    # === ZONA DE PREVISIÓN (SUBIDA PARA QUE NO SE CORTE) ===
    # Línea divisoria (Subida al 18%)
    plt.plot([0.05, 0.95], [0.18, 0.18], color='#334155', linewidth=1, transform=ax.transAxes)
    
    # Texto (Subido al 8% de altura)
    ...
    plt.text(0.05, 0.08, f_3h, color='#94a3b8', fontsize=9, fontweight='bold', ha='left', transform=ax.transAxes)
    plt.text(0.95, 0.08, f_6h, color='#94a3b8', fontsize=9, fontweight='bold', ha='right', transform=ax.transAxes)

    # Pad inches aumentado a 0.2 para dar margen negro extra
