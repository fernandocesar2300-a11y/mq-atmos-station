"""
═══════════════════════════════════════════════════════════════════════════
MQ ATMOS LAB: BELLATOR V19.1 FIXED
═══════════════════════════════════════════════════════════════════════════
CHANGELOG V19.1 FIXED:
✅ FIX #1: API keys moved to environment variables (secrets)
✅ FIX #2: matplotlib set_facecolor() call syntax corrected (×3)
✅ FIX #3: Hourly index clamped to max 47 (no out-of-range)
✅ FIX #4: get_hourly_data() extracted outside sector loop
✅ KEEP: V19.0 RESILIENT architecture 100% intacto
...
print("📡 INICIANDO SISTEMA V19.1 FIXED (PARALLEL + HEALTH MONITORING)...")

# ═══════════════════════════════════════════════════════════════════════════
# API CREDENTIALS — FIX #1: desde environment variables
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# NASA POWER API - PARALLEL EXECUTION
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# GET HOURLY DATA — FIX #3 + FIX #4: fuera del loop, índice clamped
# ═══════════════════════════════════════════════════════════════════════════

def get_hourly_data(r, sec, h, nasa_irradiance_cache):
    """
    Extrae datos horarios para índice h (clamped 0-47).
    FIX #3: min(h, 47) evita IndexError en horas >21 UTC.
    FIX #4: función definida una sola vez, fuera del loop de sectores.
    """
    h = min(h, 47)
...
def generate_ui_card(sector, data_now, data_3h, data_6h, time_str):
    ...
    ax.set_facecolor('#0f172a')  # FIX #2: paréntesis, no asignación
...
def generate_dashboard_banner(status, min_eei, max_wind, worst_sector, time_str, snow_detected):
    ...
    ax_radar.set_facecolor('#0a0a0a')  # FIX #2
...
for sec in sectors:
    try:
        url = (
            ...
            f"relativehumidity_2m,global_tilted_irradiance,snowfall,freezing_level_height"
            f"&forecast_days=2"
        )
        r = requests.get(url, timeout=10).json()

        # FIX #3 + #4: usar get_hourly_data() con índice clamped
...
status_data = {
    ...
    "event": "MQ2026",
    "model_version": "MQ Rider Index v3.1 | V19.1 FIXED",
    "data_sources": [
    ...
    "fixes_v191": [
        ...
        "matplotlib set_facecolor() syntax corrected",
        "Hourly index clamped to max 47",
        "get_hourly_data() extracted outside sector loop"
    ],
    "usage": {
