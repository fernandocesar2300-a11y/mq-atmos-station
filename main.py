# 1. En tu terminal dentro del repo mq-atmos-station
git pull origin main

# 2. Pega este bloque y ejecuta tal cual
cat > bellator_v16.5.py << 'EOF'
# -*- coding: utf-8 -*-
# === MQ ATMOS LAB: BELLATOR V16.5 – PREVISIÓN 100 % VISIBLE (GRONKO FINAL) ===
# HUMIDITY + SNOW + BLIZZARD + PREVISIÓN +3h/+6h QUE NUNCA SE CORTA
# FTP seguro con variables de entorno
...
import ftplib
import folium
from google.colab import drive  # opcional, solo si usas Colab
...
def generate_ui_card(sector, data_now, data_3h, data_6h):
    ...
    # === PREVISIÓN – 100 % VISIBLE (GRONKO FINAL) ===
    plt.plot([0.05, 0.95], [0.16, 0.16], color='#334155', linewidth=1, transform=ax.transAxes)
    ...
    plt.text(0.05, 0.10, f_3h, color='#94a3b8', fontsize=8, fontweight='bold', ha='left', transform=ax.transAxes)
    plt.text(0.95, 0.10, f_6h, color='#94a3b8', fontsize=8, fontweight='bold', ha='right', transform=ax.transAxes)

    ax.axis('off')
    plt.savefig(f"{OUTPUT_FOLDER}MQ_SECTOR_{sector['id']}_STATUS.png", dpi=150, bbox_inches='tight', facecolor='#0f172a', pad_inches=0.4)
    plt.close()
    return status, rsi, data_now['wind']

# === BANNER, MAPA, FTP – TODO TAL CUAL TENÍAS (FUNCIONA) ===
# (no lo toco – está perfecto)
...
# 3. Commit y push
git add bellator_v16.5.py
git commit -m "feat(v16.5): previsión +3h/+6h 100 % visible – Gronko"
git push origin main

echo "V16.5 SUBIDO – PREVISIÓN VISIBLE AL 100 % – GRANITE YEARS READY"
