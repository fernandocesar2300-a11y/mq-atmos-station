# -*- coding: utf-8 -*-
# === MQ ATMOS LAB: BELLATOR V16.3 (TACTICAL TEXT) ===
# SOLUCIÓN: Reemplazo de Emojis por TEXTO (RAIN, CLEAR) para evitar error de fuentes en Linux.
# ESTÉTICA: Panel de control militar (Sin iconos de colores, solo datos).
...
print("📡 INICIANDO SISTEMA V16.3 (TEXT MODE)...")
...
# 3. LÓGICA METEO (TEXTO EN LUGAR DE ICONOS)
def get_weather_text(code):
    if code == 0: return "CLEAR"      # Antes ☀️
    if 1 <= code <= 3: return "CLOUDY"   # Antes ☁️
    if code in [45, 48]: return "FOG"    # Antes 🌫️
    if 51 <= code <= 67: return "RAIN"   # Antes 🌧️
    if code in [71,73,75,77,85,86]: return "SNOW" # Antes ❄️
    if 80 <= code <= 82: return "STORM"  # Antes ⛈️
    if 95 <= code <= 99: return "THUNDER" # Antes ⚡
    return "OVCAST"
...
def calculate_mq_rsi(temp, wind, humidity, altitude, rain, gtype, code):
    ...
    if rain > 0.5: rsi -= 6.0
    if temp > 25: rsi += (humidity / 100) * 5.0 # Parche Humedad
...
def generate_ui_card(sector, data_now, data_3h, data_6h):
    ...
    # Flechas de texto simple (ASCII) compatible con Linux
    def get_arrow(curr, fut):
        diff = fut - curr
        if diff < -2: return "(-)" # Bajando
        if diff > 2: return "(+)" # Subiendo
        return "(=)" # Igual
    ...
    # TEXTO GRANDE DE FONDO (En lugar de icono)
    if is_snow: 
    ...
    plt.text(0.05, 0.04, f_3h, color='#94a3b8', fontsize=9, fontweight='bold', ha='left', transform=ax.transAxes)
    plt.text(0.95, 0.04, f_6h, color='#94a3b8', fontsize=9, fontweight='bold', ha='right', transform=ax.transAxes)

    # Guardar con ajuste para no cortar texto
    ax.axis('off'); plt.savefig(f"{OUTPUT_FOLDER}MQ_SECTOR_{sector['id']}_STATUS.png", dpi=150, bbox_inches='tight', facecolor='#0f172a', pad_inches=0.2); plt.close()
...
for sec in sectors:
    try:
        ...
        def get_data_at_hour(response, h_idx, altitude):
            d = {'temp': response['hourly']['temperature_2m'][h_idx], 'wind': response['hourly']['windspeed_10m'][h_idx], 'rain': response['hourly']['precipitation'][h_idx], 'hum': response['hourly']['relativehumidity_2m'][h_idx], 'code': response['hourly']['weathercode'][h_idx]}
...
print("✅ SISTEMA V16.3 COMPLETADO.")
